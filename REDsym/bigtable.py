...
def create_DB():
    ...
    TABLES_RED['AlbumsArtists_RED'] = dedent('''\
        CREATE TABLE `AlbumsArtists_RED` (
          ...
          UNIQUE KEY `AlbumsArtists_TOTALindex_RED` (
            `ArtistId`, `TorrentGroupId`, `musicInfo`),
    ...
    TABLES_RED['CollageAlbum_RED'] = dedent('''\
        CREATE TABLE `CollageAlbum_RED` (
          `CollageId` int(11) NOT NULL,
          `TorrentGroupId` int(11) NOT NULL,
          UNIQUE KEY `CollageAlbum_TOTALindex_RED` (
            `CollageId`, `TorrentGroupId`),
    ...
    TABLES_WCD['AlbumsArtists_WCD'] = dedent('''\
        CREATE TABLE `AlbumsArtists_WCD` (
          ...
          `TorrentGroupId` int(11) NOT NULL,
          `musicInfo` varchar(191),
          UNIQUE KEY `AlbumsArtists_TOTALindex_WCD` (
            `ArtistId`, `TorrentGroupId`, `musicInfo`),
    ...
    TABLES_WCD['CollageAlbum_WCD'] = dedent('''\
        CREATE TABLE `CollageAlbum_WCD` (
          `CollageId` int(11) NOT NULL,
          `TorrentGroupId` int(11) NOT NULL,
          UNIQUE KEY `CollageAlbum_TOTALindex_WCD` (
            `CollageId`, `TorrentGroupId`),
...
class DBase:
    ...
    def get_meta_from_dir_WCD(self, dir):
        ...
        filelist = re.split(r'\{\{\{[0-9]+\}\}\}\|+|\{\{\{[0-9]+\}\}\}+',
                            row[0][7])
        filelist_no_empty_items = list(filter(None, filelist))

        groupid_select_meta_album = {
            'format': row[0][0],
            'remasterYear': row[0][1],
            'remastered': row[0][1],
            'remasterTitle': row[0][3],
            'dir': row[0][4],
            'filePath': row[0][5],
            'remasterCatalogueNumber': row[0][6],
            'fileList': filelist_no_empty_items,
            'name': row[0][8],
            'year': row[0][9],
            'TorrentGroupId': row[0][10],
            'TorrentId': row[0][11],
        }

        sql = dedent('''\
                     SELECT aa.musicInfo, ar.name FROM AlbumsArtists_WCD AS aa
                     JOIN Artists_WCD AS ar ON aa.ArtistId=ar.ArtistId
                     WHERE aa.TorrentGroupId = %s
                     ''')
    ...
    def get_meta_from_dir_RED(self, dir):
        ...
        sql = dedent('''\
                     SELECT aa.musicInfo, ar.name FROM AlbumsArtists_RED AS aa
                     JOIN Artists_RED AS ar ON aa.ArtistId=ar.ArtistId
                     WHERE aa.TorrentGroupId = %s
                     ''')
        groupid = groupid_select_meta_album.get('TorrentGroupId')
        self.cur.execute(sql, (groupid,))
